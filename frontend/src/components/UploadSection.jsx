...
const UploadSection = React.memo(function UploadSection({ 
  ...
  return (
    <section className="w-full space-y-5" aria-labelledby="upload-hub-title">
      ...
      {ocrError && !isLoading && (
        <div 
          className="flex items-start gap-3 rounded-2xl bg-red-500/10 border border-red-500/20 p-4 text-left w-full shadow-lg shadow-red-950/5 animate-fade-in"
          role="alert"
          aria-live="assertive"
        >
      ...
      {!fileName && !imagePreview && !isLoading ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-fade-in">
          ...
          <div className="lg:col-span-2">
            <div
              ...
              className="group relative flex flex-col items-center justify-center backdrop-blur-xl border-2 border-dashed rounded-3xl p-6 sm:p-8 text-center transition-all duration-300 cursor-pointer min-h-[240px] sm:min-h-[280px] focus:outline-none focus:ring-2 focus:ring-violet-500 border-slate-800/80 hover:shadow-2xl hover:shadow-violet-500/5 hover:scale-[1.002]"
            >
              <input
                ref={fileInputRef}
                type="file"
              ...
              <div 
                className="flex h-16 w-16 items-center justify-center rounded-2xl bg-slate-950/80 border border-slate-800 text-violet-400 shadow-xl group-hover:scale-105 group-hover:border-violet-500/40 group-hover:text-violet-300 transition-all duration-300 animate-bounce"
        ...
        <div 
          className="backdrop-blur-xl bg-slate-900/15 border border-slate-800/40 rounded-3xl p-6 sm:p-8 flex flex-col items-center justify-center gap-6 min-h-[240px] sm:min-h-[280px] w-full relative overflow-hidden animate-fade-in"
          role="status"
          aria-live="polite"
        >
          ...
          <div className="w-full max-w-sm space-y-2 z-10">
            ...
            <div className="relative h-3 w-full rounded-full bg-slate-950 border border-slate-800/80 overflow-hidden" aria-valuenow={Math.round(ocrProgress)} aria-valuemin="0" aria-valuemax="100">
              <div 
                className="h-full rounded-full bg-gradient-to-r from-violet-600 via-indigo-500 to-cyan-400 shadow-[0_0_15px_rgba(139,92,246,0.5)] transition-all duration-300 ease-out"
        ...
        <div className="w-full backdrop-blur-xl bg-slate-900/15 border border-slate-800/40 rounded-3xl p-5 md:p-6 flex flex-col md:flex-row items-center justify-between gap-5 shadow-2xl relative overflow-hidden animate-fade-in">
          ...
          <div className="flex flex-col sm:flex-row items-center gap-4 w-full md:w-auto">
            <div 
              className={`flex h-16 w-16 shrink-0 items-center justify-center rounded-2xl text-white shadow-xl transition-all duration-300 hover:scale-105 hover:rotate-2 ${
                detectedType === 'PDF' 
          ...
          <div className="flex items-center gap-2.5 w-full sm:w-auto shrink-0 justify-center">
            <button
              ...
              className="flex-1 sm:flex-initial flex items-center justify-center gap-1.5 rounded-xl bg-slate-950 border border-slate-800 hover:border-violet-500/30 text-slate-300 hover:text-white px-4 py-2.5 text-xs font-semibold transition-all duration-200"
            ...
            <button
              onClick={handleClearClick}
              className="flex-1 sm:flex-initial flex items-center justify-center gap-1.5 rounded-xl bg-slate-950 hover:bg-red-500/10 border border-slate-800 hover:border-red-500/20 text-slate-400 hover:text-red-400 px-4 py-2.5 text-xs font-semibold transition-all duration-200"
